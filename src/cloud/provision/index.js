...
export async function provisionTerraform(environment, terraform, options) {
  await withDir(
    path.resolve('deployment', 'environments', environment, 'provisioning'),
    async () => {
      ...
      if (terraform == 'init') {
        ...
        try {
          ...
          if (e.message.includes('BucketNotFoundException')) {
            logger.info(
              kleur.yellow(
                `${terraformBucket} does not exist. Creating now...`,
              ),
            );
        ...
        logger.info(kleur.green(`Terraform bucket: ${terraformBucket}`));
        logger.info(
          'Initialization with -reconfigure can take several minutes...',
        );
        ...
        logger.info(kleur.green(`Terraform bucket: ${terraformBucket}`));
        logger.info(
          'Initialization with -migrate-state can take several minutes...',
        );
        ...
        try {
          ...
          logger.info(
            kleur.yellow(
              'Make sure to manually empty buckets before destroying (failsafe)',
            ),
          );
        }
      } else {
        logger.info(
          kleur.yellow(`Terraform command "${terraform}" not supported`),
        );
      }
    },
  );
